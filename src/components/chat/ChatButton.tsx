...
export default function ChatButton({ isOpen, onClick, hasUnread = false }: ChatButtonProps) {
  return (
    ...
    >
      ...
      {/* Main button */}
      <div className="relative p-4 bg-gradient-to-br from-purple-600 via-blue-600 to-cyan-600 rounded-full shadow-2xl transition-all duration-300 group-hover:scale-110 group-hover:rotate-12 focus:outline-none focus:ring-4 focus:ring-purple-300">
        {isOpen ? (
          ...
          <div className="relative">
            ...
            {hasUnread && (
              <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full animate-bounce border-2 border-white" />
            )}
          </div>
        )}
      </div>
